...
struct Linkednode* delete(struct Linkednode *  n) {
            ...
            if(size <= root->size)
            {
                if(root->size != 8) ///////minimum memory segment
                {
                    if(size <= root->size/2)
                    {
                        ...
                        root->next->right[(512/(root->size/2)) - 1] = false;
                        root->size = root->size/2;
                        root->right[(512/(root->size/2)) - 1] = true;
    ...
    while(b && child_finish==false ){
        ...
        if(t){
            ...
            else
            {
                ...
                AddToMemory(Linkedhead, newprocess->info_obj.id, newprocess->info_obj.memorySize);
                printf("after add\n");
                displayForward();
